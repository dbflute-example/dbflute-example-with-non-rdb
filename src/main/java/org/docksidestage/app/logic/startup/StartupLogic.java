...
public class StartupLogic {
    ...
    public void toNonrdb(File projectDir, String domain, String serviceName) {
        String packageName = buildPackageName(domain);
        new NewProjectCreator("nonrdb", projectDir, original -> {
            String filtered = original;
            filtered = replace(filtered, buildProjectDirPureName(projectDir), Srl.initUncap(serviceName)); // e.g. lastaflute-example-nonrdb
            filtered = replace(filtered, "lastaflute-example-nonrdb", Srl.initUncap(serviceName)); // just in case
            ...
            filtered = replace(filtered, "Nonrdb", Srl.initCap(serviceName));
            filtered = replace(filtered, "nonrdb", Srl.initUncap(serviceName));
